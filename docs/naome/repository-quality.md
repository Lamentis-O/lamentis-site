...
Adapters are plug-and-play profiles such as `rust`, `javascript-typescript`,
or `angular`. They add stack-specific ignored/generated paths and path rules at
runtime without hard-coding a specific product repository into the generic
template.
...
- `angular`: Angular CLI, Nx, `project.json`, `src/app`, `apps/*/src`, and
  `libs/*/src` layouts. It treats `.angular`, `dist`, generated clients, specs,
  components, services, guards, pipes, and templates with Angular-specific
  limits.
