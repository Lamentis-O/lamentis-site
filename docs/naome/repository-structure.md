...
`javascript-typescript`, `angular`, `swift`, `xcode`, `xctest`, `swiftui`,
`ios-app-structure`, `swift-package`, `ios-resources`, and `generated-ios`.
The Angular adapter recognizes Angular CLI, Nx, `project.json`, `apps/*/src`,
`libs/*/src`, specs, generated clients, `.angular`, `dist`, and coverage
outputs. Future adapters can add source roots, test roots, module roots, and
allowed root files without changing gate behavior.
